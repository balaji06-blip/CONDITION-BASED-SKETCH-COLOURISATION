# ================================================================
# STABLE STRONG CONDITIONAL DIFFUSION FOR ANIME SKETCH COLOURISATION
# Palette-inspired conditional DDPM training + DDIM sampling
...
OUTPUT_ROOT = os.path.join(PROJECT_ROOT, "diffusion_palette_stable_100epochs")
...
IMAGE_SIZE = 256
HINT_RATE = 0.05              # FAIR comparison with your GAN baseline
...
class AnimeSketchColourDataset(Dataset):
    ...
    Returns:
      ...
      hint_rgb    : 3xHxW; normalized colours only at hint locations, 0 elsewhere
      hint_mask   : 1xHxW; 1 at hint pixels, 0 elsewhere
      condition   : 7xHxW = sketch(3) + hint_rgb(3) + mask(1)
    """
    ...
    def __getitem__(self, index):
        ...
        # Sparse colour hints, same information budget as baseline.
        ...
        hint_np = np.zeros_like(colour_norm_np, dtype=np.float32)
        hint_mask_np = np.zeros((h, w), dtype=np.float32)

        hint_np[rows, cols, :] = colour_norm_np[rows, cols, :]
        hint_mask_np[rows, cols] = 1.0

        hint_rgb = torch.from_numpy(np.transpose(hint_np, (2, 0, 1)).copy()).float()
        hint_mask = torch.from_numpy(hint_mask_np[None, :, :].copy()).float()

        condition = torch.cat([sketch_tensor, hint_rgb, hint_mask], dim=0)
...
class ConditionEncoder(nn.Module):
    ...
    def __init__(self, cond_ch=7, base=64):
        super().__init__()
...
class StrongConditionalUNet(nn.Module):
    def __init__(self, base=64, cond_ch=7, out_ch=3,
...
def diffusion_training_loss(model, schedule, batch):
    ...
    # Only the network forward pass uses mixed precision. All loss arithmetic
    # is explicitly FP32 to prevent half-precision overflow/underflow.
    with autocast_context():
    ...
    if not torch.isfinite(pred_noise_f).all():
        raise FloatingPointError(
            ...
            "Training was stopped before corrupting a checkpoint."
        )

    # Min-SNR weighted epsilon MSE, trained over ALL timesteps.
    ...
    # Reconstruct x0 in FP32. At extremely low SNR, x0 reconstruction is
    # ill-conditioned, so the auxiliary reconstruction/hint losses are masked
    # there. The main epsilon objective still uses those high-noise samples.
    ...
    if not torch.isfinite(total_loss):
        raise FloatingPointError(
            ...
            "Training was stopped before an invalid optimizer step."
        )
...
def train_one_epoch(model, ema_model, schedule, loader, optimizer,
...
@torch.no_grad()
def validate_model(model, schedule, loader, ddim_steps):
    model.eval()
...
@torch.no_grad()
def save_sample_grid(model, schedule, loader, epoch):
    model.eval()
    ...
    for i in range(n):
        ...
        # Display sparse hint image on white background for readability.
        ...
        row = np.concatenate([sketch_img, hint_img, generated_img, target_img], axis=1)
        rows.append(row)
...
def save_training_curves(history):
    ...
    if val_epochs:
        ...
        plt.title("Diffusion Validation PSNR")
        plt.grid(alpha=0.3)
        ...
        plt.title("Diffusion Validation SSIM")
        plt.grid(alpha=0.3)
...
@torch.no_grad()
def evaluate_test(model, schedule, loader):
    model.eval()
    ...
    for batch_index, batch in enumerate(tqdm(loader, desc="Final test evaluation")):
        target = batch["target"].to(DEVICE)
        condition = batch["condition"].to(DEVICE)
        hint_rgb = batch["hint_rgb"].to(DEVICE)
        hint_mask = batch["hint_mask"].to(DEVICE)
    ...
    summary = {
        "model": "Strong Conditional Diffusion (Palette-style)",
        "epochs": TRAIN_EPOCHS,
        "hint_rate": HINT_RATE,
        ...
        "gan_baseline": {
            "psnr_mean": 24.27932790471955,
            "ssim_mean": 0.8783355475609408,
            "lpips_mean": 0.09930565163337673,
            "infer_ms": 1.0922189231391426,
        },
    }
    ...
    with open(os.path.join(EVAL_DIR, "summary_test.txt"), "w", encoding="utf-8") as f:
        f.write("Model: Strong Conditional Diffusion (Palette-style)\n")
        ...
        f.write(f"Inference: {average_inference_ms:.2f} ms/image\n")
...
def save_result_grid(results, filename, title, number=8):
    ...
    titles = ["Sketch", "Colour Hints", "Generated", "Ground Truth"]
    for col, text in enumerate(titles):
        axes[0, col].set_title(text, fontweight="bold")

    for row, result in enumerate(results):
        images = [result["sketch"], result["hint"], result["generated"], result["target"]]
...
def main():
    print("\n" + "=" * 80)
    print("STRONG CONDITIONAL DIFFUSION - ANIME SKETCH COLOURISATION")
    print("Palette-style conditional U-Net + attention + EMA + Min-SNR")
    print("=" * 80)
    print("Device              :", DEVICE)
    if torch.cuda.is_available():
    ...
    for epoch in range(start_epoch, TRAIN_EPOCHS + 1):
        ...
        if (
            ...
            val_psnr, val_ssim = validate_model(
                ema_model, schedule, val_loader, VAL_DDIM_STEPS
            )
        ...
        if epoch <= MONITOR_FIRST_N_EPOCHS or epoch % SAMPLE_EVERY == 0:
            sample_path = save_sample_grid(ema_model, schedule, sample_loader, epoch)
    ...
    (
        ...
        inference_ms,
        visual_results,
    ) = evaluate_test(ema_model, schedule, test_loader)

    save_result_grid(
        visual_results,
        "results_grid.png",
        "Strong Conditional Diffusion Test Results",
        8,
    )
    ...
    save_result_grid(best_cases, "best_cases.png", "Best Diffusion Cases", 4)
    save_result_grid(failure_cases, "failure_cases.png", "Diffusion Failure Cases", 4)

    total_hours = (time.time() - total_training_start) / 3600.0

    print("\n" + "=" * 80)
    print("STRONG CONDITIONAL DIFFUSION COMPLETE")
    print("=" * 80)
    print("Best epoch:", best_epoch)
    print(f"Best validation PSNR: {best_psnr:.4f} dB")
    print()
    print(f"DIFFUSION TEST PSNR : {mean_psnr:.4f} +/- {std_psnr:.4f} dB")
    print(f"GAN BASELINE PSNR   : 24.2793 +/- 2.0089 dB")
    print()
    print(f"DIFFUSION TEST SSIM : {mean_ssim:.4f} +/- {std_ssim:.4f}")
    print(f"GAN BASELINE SSIM   : 0.8783 +/- 0.0450")
    print()
    if mean_lpips is not None:
        print(f"DIFFUSION TEST LPIPS: {mean_lpips:.4f} +/- {std_lpips:.4f}")
        print(f"GAN BASELINE LPIPS  : 0.0993 +/- 0.0325")
        print()
    print(f"DIFFUSION inference : {inference_ms:.2f} ms/image")
    print(f"GAN inference       : 1.09 ms/image")
